...
RegionEU433::RegionEU433(RadioHandler *r)
{
    radio = r;
}

uint32_t RegionEU433::GetBandwidth(uint32_t drIndex)
{
...
bool RegionEU433::VerifyTxFreq(uint32_t freq, uint8_t *band)
{
...
uint8_t RegionEU433::CountNbOfEnabledChannels(bool joined, uint8_t datarate, uint16_t *channelsMask, ChannelParams_t *channels, Band_t *bands, uint8_t *enabledChannels, uint8_t *delayTx)
{
...
PhyParam_t RegionEU433::RegionGetPhyParam(GetPhyParams_t *getPhy)
{
...
void RegionEU433::RegionSetBandTxDone(SetBandTxDoneParams_t *txDone)
{
	RegionCommonSetBandTxDone(txDone->Joined, &Bands[Channels[txDone->Channel].Band], txDone->LastTxDoneTime);
}

void RegionEU433::RegionInitDefaults(InitType_t type)
{
	switch (type)
	{
	case INIT_TYPE_INIT:
...
bool RegionEU433::RegionVerify(VerifyParams_t *verify, PhyAttribute_t phyAttribute)
{
	switch (phyAttribute)
	{
	case PHY_TX_DR:
...
void RegionEU433::RegionApplyCFList(ApplyCFListParams_t *applyCFList)
{
...
bool RegionEU433::RegionChanMaskSet(ChanMaskSetParams_t *chanMaskSet)
{
...
bool RegionEU433::RegionAdrNext(AdrNextParams_t *adrNext, int8_t *drOut, int8_t *txPowOut, uint32_t *adrAckCounter)
{
		...
		else
		{
			...
			if (adrNext->AdrAckCounter >= (EU433_ADR_ACK_LIMIT + EU433_ADR_ACK_DELAY))
			{
				if ((adrNext->AdrAckCounter % EU433_ADR_ACK_DELAY) == 1)
				{
                    ...
                    phyParam = RegionGetPhyParam(&getPhy);
...
void RegionEU433::RegionComputeRxWindowParameters(int8_t datarate, uint8_t minRxSymbols, uint32_t rxError, RxConfigParams_t *rxConfigParams)
{
	double tSymbol = 0.0;
...
bool RegionEU433::RegionRxConfig(RxConfigParams_t *rxConfig, int8_t *datarate)
{
	RadioModems_t modem;
    ...
    if (radio->GetStatus(radio->sxHandler) != RF_IDLE)
    {
		return false;
	}
    ...
    radio->SetChannel(frequency, radio->sxHandler);
	...
	if (dr == DR_7)
	{
        ...
        radio->SetRxConfig(50000, phyDr * 1000, 0, 83333, 5, 0, false, 0, true, 0, 0, false, rxConfig->RxContinuous, radio->sxHandler);
    }
	else
	{
		modem = MODEM_LORA;
        ...
        radio->SetRxConfig(rxConfig->Bandwidth, phyDr, 1, 0, 8, 0, false, 0, false, 0, 0, true, rxConfig->RxContinuous, radio->sxHandler);
    }
    ...
    radio->SetMaxPayloadLength(maxPayload + LORA_MAC_FRMPAYLOAD_OVERHEAD, radio->sxHandler);

	*datarate = (uint8_t)dr;
	return true;
}

bool RegionEU433::RegionTxConfig(TxConfigParams_t *txConfig, int8_t *txPower, TimerTime_t *txTimeOnAir)
{
	RadioModems_t modem;
    ...
    radio->SetChannel(Channels[txConfig->Channel].Frequency, radio->sxHandler);

	if (txConfig->Datarate == DR_7)
	{ // High Speed FSK channel
		modem = MODEM_FSK;
        //Radio.SetTxConfig(modem, phyTxPower, 25000, bandwidth, phyDr * 1000, 0, 5, false, true, 0, 0, false, 3000);
        radio->SetTxConfig(phyTxPower, 25000, bandwidth, phyDr * 1000, 0, 5, false, true, 0, 0, false, 3000, radio->sxHandler);
    }
	else
	{
		modem = MODEM_LORA;
        //Radio.SetTxConfig(modem, phyTxPower, 0, bandwidth, phyDr, 1, 8, false, true, 0, 0, false, 3000);
        radio->SetTxConfig(phyTxPower, 0, bandwidth, phyDr, 1, 8, false, true, 0, 0, false, 3000, radio->sxHandler);

    }

	// Setup maximum payload lenght of the radio driver
    //Radio.SetMaxPayloadLength(modem, txConfig->PktLen);
    radio->SetMaxPayloadLength(txConfig->PktLen, radio->sxHandler);
	// Get the time-on-air of the next tx frame
    //*txTimeOnAir = Radio.TimeOnAir(modem, txConfig->PktLen);
    *txTimeOnAir = radio->TimeOnAir(txConfig->PktLen);
...
uint8_t RegionEU433::RegionLinkAdrReq(LinkAdrReqParams_t *linkAdrReq, int8_t *drOut, int8_t *txPowOut, uint8_t *nbRepOut, uint8_t *nbBytesParsed)
{
	uint8_t status = 0x07;
...
uint8_t RegionEU433::RegionRxParamSetupReq(RxParamSetupReqParams_t *rxParamSetupReq)
{
	uint8_t status = 0x07;
...
uint8_t RegionEU433::RegionNewChannelReq(NewChannelReqParams_t *newChannelReq)
{
	uint8_t status = 0x03;
	ChannelAddParams_t channelAdd;
	ChannelRemoveParams_t channelRemove;
	...
	if (newChannelReq->NewChannel->Frequency == 0)
	{
		...
		// Remove
        if (RegionChannelsRemove(&channelRemove) == false)
		{
			status &= 0xFC;
		}
	}
	else
	{
        ...
        switch (RegionChannelAdd(&channelAdd))
		{
...
int8_t RegionEU433::RegionTxParamSetupReq(TxParamSetupReqParams_t *txParamSetupReq)
{
	return -1;
}

uint8_t RegionEU433::RegionDlChannelReq(DlChannelReqParams_t *dlChannelReq)
{
	uint8_t status = 0x03;

	// Verify if the frequency is supported
    if (VerifyTxFreq(dlChannelReq->Rx1Frequency, 0) == false)
	{
		status &= 0xFE;
	}
...
int8_t RegionEU433::RegionAlternateDr(AlternateDrParams_t *alternateDr)
{
	int8_t datarate = 0;
...
void RegionEU433::RegionCalcBackOff(CalcBackOffParams_t *calcBackOff)
{
...
bool RegionEU433::RegionNextChannel(NextChanParams_t *nextChanParams, uint8_t *channel, TimerTime_t *time, TimerTime_t *aggregatedTimeOff)
{
	uint8_t nbEnabledChannels = 0;
	uint8_t delayTx = 0;
...
LoRaMacStatus_t RegionEU433::RegionChannelAdd(ChannelAddParams_t *channelAdd)
{
	uint8_t band = 0;
	...
	{
        if (VerifyTxFreq(channelAdd->NewChannel->Frequency, 0) == false)
		{
			freqInvalid = true;
		}
	}
...
bool RegionEU433::RegionChannelsRemove(ChannelRemoveParams_t *channelRemove)
{
...
void RegionEU433::RegionSetContinuousWave(ContinuousWaveParams_t *continuousWave)
{
    ...
    radio->SetTxContinuousWave(frequency, phyTxPower, continuousWave->Timeout, radio->sxHandler);
}

uint8_t RegionEU433::RegionApplyDrOffset(uint8_t downlinkDwellTime, int8_t dr, int8_t drOffset)
{
