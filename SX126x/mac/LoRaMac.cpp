...
LoraMac::LoraMac(LoraMacHelper *mac, RadioHandler *r, RegionBase *b)
{
    this->radio = r;
    this->radioRegion = b;
    this->macHelper = mac;
}
LoRaMacStatus_t LoraMac::LoRaMacInitialization(LoRaMacPrimitives_t *primitives, LoRaMacCallback_t *callbacks, LoRaMacRegion_t region, eDeviceClass nodeClass)
{
...
*/
    ...
    phyParam = radioRegion->RegionGetPhyParam(&getPhy);
    ...
    radioRegion->RegionInitDefaults(INIT_TYPE_INIT);
    ...
    // Initialize timers
    TimerInit(&MacStateCheckTimer, std::bind(&LoraMac::OnMacStateCheckTimerEvent, this));
    TimerSetValue(&MacStateCheckTimer, MAC_STATE_CHECK_TIMEOUT);

    TimerInit(&TxDelayedTimer, std::bind(&LoraMac::OnTxDelayedTimerEvent, this));
    TimerInit(&RxWindowTimer1, std::bind(&LoraMac::OnRxWindow1TimerEvent, this));
    TimerInit(&RxWindowTimer2, std::bind(&LoraMac::OnRxWindow2TimerEvent, this));
    TimerInit(&AckTimeoutTimer, std::bind(&LoraMac::OnAckTimeoutTimerEvent, this));
    ...
    radio->Init(&RadioEvents, radio->sxHandler);

    // Random seed initialization
    //srand1(Radio.Random());
    srand1(radio->Random(radio->sxHandler));

    PublicNetwork = true;
    //Radio.SetPublicNetwork(PublicNetwork);
    radio->SetPublicNetwork(PublicNetwork, radio->sxHandler);
    ...
    radioRegion->RegionTxConfig(&txConfig, &txPower, &TxTimeOnAir);

    //Radio.Sleep();
    radio->Sleep(radio->sxHandler);

    return LORAMAC_STATUS_OK;
}

void LoraMac::MacMcpsConfirm(std::function<void (McpsConfirm_t *)> callback, McpsConfirm_t* McpsConfirm)
{
    callback(McpsConfirm);
}

void LoraMac::MacMcpsIndication(std::function<void (McpsIndication_t *)> callback, McpsIndication_t *McpsIndication)
{
    callback(McpsIndication);
}

void LoraMac::MacMlmeConfirm(std::function<void (MlmeConfirm_t *)> callback, MlmeConfirm_t *MlmeConfirm)
{
    callback(MlmeConfirm);
}

uint32_t LoraMac::LoRaMacGetOTAADevId()
{
...
LoRaMacStatus_t LoraMac::LoRaMacQueryTxPossible(uint8_t size, LoRaMacTxInfo_t *txInfo)
{
    ...
    radioRegion->RegionAdrNext(&adrNext, &datarate, &txPower, &AdrAckCounter);
...
LoRaMacStatus_t LoraMac::LoRaMacChannelAdd(uint8_t id, ChannelParams_t params)
{
    ...
    return radioRegion->RegionChannelAdd(&channelAdd);
}
...
LoRaMacStatus_t LoraMac::LoRaMacChannelRemove(uint8_t id)
{
    ...
    if (radioRegion->RegionChannelsRemove(&channelRemove) == false)
    {
        return LORAMAC_STATUS_PARAMETER_INVALID;
    }
    return LORAMAC_STATUS_OK;
...
LoRaMacStatus_t LoraMac::LoRaMacMibSetRequestConfirm(MibRequestConfirm_t *mibSet)
{
    ...
    case MIB_RX2_CHANNEL:
    {
        ...
        if (radioRegion->RegionVerify(&verify, PHY_RX_DR) == true)
        {
            ...
            if ((LoRaMacDeviceClass == CLASS_C) && (IsLoRaMacNetworkJoined == JOIN_OK))
            {
                ...
                radioRegion->RegionComputeRxWindowParameters(LoRaMacParams.Rx2Channel.Datarate,
                ...
                if (radioRegion->RegionRxConfig(&RxWindow2Config, (int8_t *)&McpsIndication.RxDatarate) == true)
                {
    ...
    case MIB_CHANNELS_DEFAULT_MASK:
    {
        ...
        if (radioRegion->RegionChanMaskSet(&chanMaskSet) == false)
        {
            status = LORAMAC_STATUS_PARAMETER_INVALID;
        }
        break;
    ...
    case MIB_CHANNELS_DEFAULT_DATARATE:
    {
        ...
        if (radioRegion->RegionVerify(&verify, PHY_DEF_TX_DR) == true)
        {
    ...
    case MIB_CHANNELS_DATARATE:
    {
        ...
        if (radioRegion->RegionVerify(&verify, PHY_TX_DR) == true)
        {
    ...
    case MIB_CHANNELS_DEFAULT_TX_POWER:
    {
        ...
        if (radioRegion->RegionVerify(&verify, PHY_DEF_TX_POWER) == true)
        {
    ...
    case MIB_CHANNELS_TX_POWER:
    {
        ...
        if (radioRegion->RegionVerify(&verify, PHY_TX_POWER) == true)
        {
...
LoRaMacStatus_t LoraMac::LoRaMacMlmeRequest(MlmeReq_t *mlmeRequest)
{
    ...
    case MLME_JOIN:
    {
        ...
        LoRaMacParams.ChannelsDatarate = radioRegion->RegionAlternateDr(&altDr);
...
void LoraMac::ResetMacParameters(void)
{
    ...
    radioRegion->RegionInitDefaults(INIT_TYPE_APP_DEFAULTS);
...
void LoraMac::OnRadioTxDone(void)
{
    ...
    radioRegion->RegionSetBandTxDone(&txDone);
...
void LoraMac::OnRadioRxDone(uint8_t *payload, uint16_t size, int16_t rssi, int8_t snr)
{
    ...
    case FRAME_TYPE_JOIN_ACCEPT:
        ...
        if (micRx == mic)
        {
            ...
            radioRegion->RegionApplyCFList(&applyCFList);
...
void LoraMac::OnMacStateCheckTimerEvent(void)
{
    ...
    if (LoRaMacFlags.Bits.MacDone == 1)
    {
        ...
        { // Retransmissions procedure for confirmed uplinks
            ...
            {
                radioRegion->RegionInitDefaults(INIT_TYPE_RESTORE);
    ...
    if (LoRaMacState == LORAMAC_IDLE)
    {
        ...
        {
            //LoraMacHelper* loraHelper = new LoraMacHelper;
            //LoRaMacPrimitives->MacMcpsConfirm(&McpsConfirm);
            MacMcpsConfirm(std::bind(&LoraMacHelper::McpsConfirm, macHelper, std::placeholders::_1), &McpsConfirm);
        ...
        if (LoRaMacFlags.Bits.MlmeReq == 1)
        {
            ...
            MacMlmeConfirm(std::bind(&LoraMacHelper::MlmeConfirm, macHelper, std::placeholders::_1), &MlmeConfirm);
    ...
    if (LoRaMacFlags.Bits.McpsInd == 1)
    {
        ...
        if (LoRaMacFlags.Bits.McpsIndSkip == 0)
        {
            ...
            MacMcpsIndication(std::bind(&LoraMacHelper::McpsIndication, macHelper, std::placeholders::_1), &McpsIndication);
...
void LoraMac::OnRxWindow1TimerEvent(void)
{
    ...
    {
        //Radio.Standby();
        radio->Standby(radio->sxHandler);
    }

    radioRegion->RegionRxConfig(&RxWindow1Config, (int8_t *)&McpsIndication.RxDatarate);
...
void LoraMac::RxWindowSetup(bool rxContinuous, uint32_t maxRxWindow)
{
    if (rxContinuous == false)
    {
        ...
        radio->Rx(maxRxWindow, radio->sxHandler);
    }
    else
    {
        //Radio.Rx(0); // Continuous mode
        radio->Rx(0, radio->sxHandler);
    }
}
...
void LoraMac::ProcessMacCommands(uint8_t *payload, uint8_t macIndex, uint8_t commandsSize, uint8_t snr)
{
    ...
    while (macIndex < commandsSize)
    {
        ...
        case SRV_MAC_LINK_ADR_REQ:
        {
            ...
            status = radioRegion->RegionLinkAdrReq(&linkAdrReq, &linkAdrDatarate,
        ...
        case SRV_MAC_RX_PARAM_SETUP_REQ:
        {
            ...
            status = radioRegion->RegionRxParamSetupReq(&rxParamSetupReq);
        ...
        case SRV_MAC_NEW_CHANNEL_REQ:
        {
            ...
            status = radioRegion->RegionNewChannelReq(&newChannelReq);
        ...
        case SRV_MAC_TX_PARAM_SETUP_REQ:
        {
            ...
            if (radioRegion->RegionTxParamSetupReq(&txParamSetupReq) != -1)
            {
                // Accept command
        ...
        case SRV_MAC_DL_CHANNEL_REQ:
        {
            ...
            status = radioRegion->RegionDlChannelReq(&dlChannelReq);
...
LoRaMacStatus_t LoraMac::ScheduleTx(void)
{
    ...
    // Select channel
    while (radioRegion->RegionNextChannel(&nextChan, &Channel, &dutyCycleTimeOff, &AggregatedTimeOff) == false)
    {
    ...
    radioRegion->RegionComputeRxWindowParameters(radioRegion->RegionApplyDrOffset(LoRaMacParams.DownlinkDwellTime, LoRaMacParams.ChannelsDatarate, LoRaMacParams.Rx1DrOffset),
                                                                                  LoRaMacParams.MinRxSymbols,
                                                                                  LoRaMacParams.SystemMaxRxError,
                                                                                  &RxWindow1Config);
    // Compute Rx2 windows parameters
...
void LoraMac::CalculateBackOff(uint8_t channel)
{
    ...
    radioRegion->RegionCalcBackOff(&calcBackOff);
...
LoRaMacStatus_t LoraMac::PrepareFrame(LoRaMacHeader_t *macHdr, LoRaMacFrameCtrl_t *fCtrl, uint8_t fPort, void *fBuffer, uint16_t fBufferSize)
{
    ...
    case FRAME_TYPE_JOIN_REQ:
        ...
        LoRaMacDevNonce = radio->Random(radio->sxHandler);
    ...
    case FRAME_TYPE_DATA_UNCONFIRMED_UP:
        ...
        fCtrl->Bits.AdrAckReq = radioRegion->RegionAdrNext(&adrNext,
...
LoRaMacStatus_t LoraMac::SendFrameOnChannel(uint8_t channel)
{
    ...
    radio->Send(LoRaMacBuffer, LoRaMacBufferPktLen, radio->sxHandler);
...
LoRaMacStatus_t LoraMac::SetTxContinuousWave(uint16_t timeout)
{
    ...
    radioRegion->RegionSetContinuousWave(&continuousWave);
...
LoRaMacStatus_t LoraMac::SetTxContinuousWave1(uint16_t timeout, uint32_t frequency, uint8_t power)
{
    ...
    radio->SetTxContinuousWave(frequency, power, timeout, radio->sxHandler);
...
void LoraMac::LoRaMacTestSetDutyCycleOn(bool enable)
{
    ...
    if (radioRegion->RegionVerify(&verify, PHY_DUTY_CYCLE) == true)
    {
        DutyCycleOn = enable;
    }
}

/*oid LoRaMacTestSetChannel(uint8_t channel)
{
    Channel = channel;
}*/
