...
public:
    LoraMacHelper(SPIBase *spi);

    void SetSpiLora(SPIBase *spi);
    ...
    bool compliance_test_tx(void);

    void OnComplianceTestTxNextPacketTimerEvent(void);
...
protected:
    ...
    uint8_t DevEui[8] = LORAWAN_DEVICE_EUI;         /**< End-device identifier */
    uint8_t AppKey[16] = LORAWAN_APPLICATION_KEY;   /**< Application key */
    uint8_t AppEui[8] = LORAWAN_APPLICATION_EUI;    /**< Application identifier */

    MlmeReqJoin_t JoinParameters;

    uint8_t NwkSKey[16] = LORAWAN_NWKSKEY;          /**< Network session key */
    uint8_t AppSKey[16] = LORAWAN_APPSKEY;          /**< Application session key */
    uint32_t DevAddr = LORAWAN_DEVICE_ADDRESS;      /**< End-device address */
    ...
    LoRaMacPrimitives_t LoRaMacPrimitives; /**< LoRaMAC events variable */
    LoRaMacCallback_t LoRaMacCallbacks;	  /**< LoRaMAC callback variable */
    MibRequestConfirm_t mibReq;			  /**< LoRaMAC MIB-RequestConfirm variable */

    lmh_param_t m_param;
    lmh_callback_t *m_callbacks;
    lmh_compliance_test_t m_compliance_test; /**< LoRaWAN compliance tests data */

    bool m_adr_enable_init;
    TimerEvent_t ComplianceTestTxNextPacketTimer;

    RadioHandler *radio;
    RegionBase *radioRegion;
    LoraMac *loraMac;
};
